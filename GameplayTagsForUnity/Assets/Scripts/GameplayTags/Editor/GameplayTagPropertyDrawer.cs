...
namespace werignac.GameplayTags.Editor
{
    ...
    public class GameplayTagPropertyDrawer : PropertyDrawer
    {
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			{
				currentValue = DrawTagRows(position, 0, rowCount, UserGameplayTags.RootTags, currentValue, out int addedRows);
				rowCount += addedRows;
			}
		...
		/// <summary>
		/// Recursively draws the rows for a tag and its children, including
		/// opening children.
		/// </summary>
		/// <param name="position">Starting position of rows.</param>
		/// <param name="indent">Starting indent of rows.</param>
		/// <param name="rowNumber">The starting row relative to the starting position.</param>
		/// <param name="toDraw">The gameplay tags to draw.</param>
		/// <param name="currentValue">The current value of the GameplayTag property (determines whether to mark checkboxes).</param>
		/// <param name="_rowCount">Output of the number of rows generated.</param>
		/// <returns>The selected gameplay tag by the user (checkmark).</returns>
